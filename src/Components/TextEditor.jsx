...
function TextEditor() {
        ...
        reader.onload = (event) => {
          mammoth
            ...
            .then((result) => {
              ...
              quillRef.current
                .getEditor()
                .setSelection(quillRef.current.getEditor().getLength(), 0);
            })
            .catch((err) => {
  ...
  useEffect(() => {
    ...
    socket &&
      socket.once("load-comment", (comment) => {
      ...
      <div className="flex justify-center gap-2 flex-wrap w-full mb-2 md:justify-end">
        <div className="bg-[#c2e7ff] flex gap-[8px] items-center text-[14px] font-[500] border-0 rounded-full px-6 py-[9.5px] relative cursor-pointer">
          <input
            ...
            className="absolute w-full h-[100%] z-[2] opacity-0 left-0 right-0 top-0 bottom-0 cursor-pointer"
          />
            ...
            <div className="flex justify-between items-center bg-[#c2e7ff] p-3 sticky top-0 z-[2] rounded-[24px] rounded-b-none gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="text-[10px] text-[#000000] border border-[#000000] px-1 py-1 rounded"
                >
                  <Close style={{ fontSize: 16 }} />
                </button>
              </div>
            </div>
            <div className="comments-data p-4 pt-8 bg-white text-[14px]">
              {comments ? (
                <div
                  dangerouslySetInnerHTML={{ __html: formatComments(comments) }}
                />
              ) : (
                "No comment found" 
              )}
            </div>
            <div className="sticky bg-white pt-[10px] flex gap-2 p-4 bottom-0 left-0 w-full z-[2]">
              <button
                type="button"
                className="bg-[#c2e7ff] text-[#000] rounded-full px-6 py-[10px] text-[16px] w-full max-w-[200px] ml-auto mr-auto"
